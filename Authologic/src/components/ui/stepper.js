  ...
  const contextValue = React.useMemo(
    () => ({
      ...
      stepsCount: React.Children.toArray(children).filter(
        (child) =>
          React.isValidElement(child) && (child.type?.displayName === 'StepperItem'),
      ).length,
      orientation,
      registerTrigger,
      focusNext,
...
function StepperIndicator({ children, className }) {
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn(
        'relative flex items-center overflow-hidden justify-center size-6 shrink-0 border-background bg-slate-300 text-slate-600 rounded-full text-xs data-[state=completed]:bg-primary data-[state=completed]:text-primary-foreground data-[state=active]:bg-primary data-[state=active]:text-primary-foreground',
        className,
      )}
    >
...
function StepperSeparator({ className }) {
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn(
        'flex-1 h-0.5 bg-slate-200 rounded-full data-[state=completed]:bg-primary mx-2',
        className,
      )}
    />
  );
...
function StepperTitle({ children, className }) {
  ...
  return (
    <h3 data-slot="stepper-title" data-state={state} className={cn('text-sm font-medium leading-none font-noto', className)}>
      {children}
    </h3>
  );
}
...
function StepperDescription({ children, className }) {
  ...
  return (
    <div data-slot="stepper-description" data-state={state} className={cn('text-sm text-muted-foreground font-noto', className)}>
      {children}
    </div>
  );
}
